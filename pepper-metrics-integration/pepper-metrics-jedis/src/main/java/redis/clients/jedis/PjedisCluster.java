 ...
 * @date 19-8-7
 * @description
 * 由于cglib动态代理的限制，目标类必须含有无参默认构造函数，考虑jedis基本稳定在2.9.0版本很多年
...
public class PjedisCluster extends PbinaryJedisCluster implements JedisCommands,
    ...
    public PjedisCluster() {
        super();
    }

    public PjedisCluster(HostAndPort node) {
        this(Collections.singleton(node), DEFAULT_TIMEOUT);
    }

    public PjedisCluster(HostAndPort node, int timeout) {
        this(Collections.singleton(node), timeout, DEFAULT_MAX_REDIRECTIONS);
    }

    public PjedisCluster(HostAndPort node, int timeout, int maxAttempts) {
        this(Collections.singleton(node), timeout, maxAttempts, new GenericObjectPoolConfig());
    }

    public PjedisCluster(HostAndPort node, final GenericObjectPoolConfig poolConfig) {
        this(Collections.singleton(node), DEFAULT_TIMEOUT, DEFAULT_MAX_REDIRECTIONS, poolConfig);
    }

    public PjedisCluster(HostAndPort node, int timeout, final GenericObjectPoolConfig poolConfig) {
    ...
    public PjedisCluster(HostAndPort node, int connectionTimeout, int soTimeout,
    ...
    public PjedisCluster(Set<HostAndPort> nodes) {
        this(nodes, DEFAULT_TIMEOUT);
    }

    public PjedisCluster(Set<HostAndPort> nodes, int timeout) {
        this(nodes, timeout, DEFAULT_MAX_REDIRECTIONS);
    }

    public PjedisCluster(Set<HostAndPort> nodes, int timeout, int maxAttempts) {
        this(nodes, timeout, maxAttempts, new GenericObjectPoolConfig());
    }

    public PjedisCluster(Set<HostAndPort> nodes, final GenericObjectPoolConfig poolConfig) {
        this(nodes, DEFAULT_TIMEOUT, DEFAULT_MAX_REDIRECTIONS, poolConfig);
    }

    public PjedisCluster(Set<HostAndPort> nodes, int timeout, final GenericObjectPoolConfig poolConfig) {
        this(nodes, timeout, DEFAULT_MAX_REDIRECTIONS, poolConfig);
    }

    public PjedisCluster(Set<HostAndPort> jedisClusterNode, int timeout, int maxAttempts,
                        final GenericObjectPoolConfig poolConfig) {
        super(jedisClusterNode, timeout, maxAttempts, poolConfig);
    }

    public PjedisCluster(Set<HostAndPort> jedisClusterNode, int connectionTimeout, int soTimeout,
                        int maxAttempts, final GenericObjectPoolConfig poolConfig) {
    ...
    public ScanResult<String> scan(final String cursor, final ScanParams params) {
        ...
        if (params == null || (matchPattern = params.match()) == null || matchPattern.isEmpty()) {
            throw new IllegalArgumentException(redis.clients.jedis.JedisCluster.class.getSimpleName() + " only supports SCAN commands with non-empty MATCH patterns");
        }
        ...
        if (JedisClusterHashTagUtil.isClusterCompliantMatchPattern(matchPattern)) {
            ...
            throw new IllegalArgumentException(redis.clients.jedis.JedisCluster.class.getSimpleName() + " only supports SCAN commands with MATCH patterns containing hash-tags ( curly-brackets enclosed strings )");
        }
    }
